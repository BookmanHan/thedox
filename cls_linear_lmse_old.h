...
template<typename T>
class cls_linear_lmse : public cls_classifier
private:
	
	cls_mat<T> mat_weight;
	cls_mat<T> mat_des;
...
public:
	...
	cls_linear_lmse(int a_feature,int a_pat)
	{
		n_feature = a_feature;
		n_pat = a_pat;
			...
			else
			{
				...
				for(int i=0;i<n_pat;i++)
				{
					for(int j=0;j<n_feature;j++)
					{
						if (i == v_des(cnt,0))
						{
							mat_weight(i,j) += v_src(cnt,j) * (1 - v_check[i])/(n_times+1);
						}
						else if (v_check[i] > v_check[v_des(cnt,0)])
						{
							mat_weight(i,j) += v_src(cnt,j) * (0 - v_check[i])/(n_times+1);
						}
					}
