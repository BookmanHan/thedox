...
class cls_parser_math_expr_wide
	:public cls_parser_wide<double>
{
private:
	vector<wstring> tokens;
	map<wstring,double> map_var_to_values;
	double d_re;

public:
	virtual void operator()(vector<cls_lalr_frame_item_wide<double> > & now,
		int next_status,
		cls_lalr_frame_item_wide<double> &re,
		vector<cls_lalr_frame_item_wide<double> >& stack) 
	{
		next_status -= 10000;
		switch(next_status)
		{
		...
		case 8:
			{
				re.user_data = get_type_from_wstring<double>(now[1].str_pre);
			}
			break;
	...
	cls_parser_math_expr_wide()
	{
		...
		frame = new cls_lalr_frame_wide<double>(table,yacc.info,yacc.get_kernel_size(),this);
	}

	virtual void set_var_values(wstring str,double val)
	{
	...
	virtual double* run( const wstring& str_context ) 
	{
		tokens.clear();
