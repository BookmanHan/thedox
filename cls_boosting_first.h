...
class cls_boosting_adaptive_first
	:public cls_boosting
{
public:
	cls_boosting_adaptive_first()
		:cls_boosting()
	{
		;
	}
		...
		for(itor = weakers.begin();itor != weakers.end();itor++)
		{
			...
			while(!ts.epoch_over())
			{
				...
				catch (cls_exception &exc)
				{
					try
					{
						int indx = itor->first->get_output_index(a);
						if (ts.get_output_for_id(indx) != b)
						{
							bs[ts.get_count()-1] = true;
							itor->second += weights[ts.get_count()-1];
						}
					...
					{
						throw cls_exception_not_sense(string(__FUNCTION__));
					}
				}
			}
	...
	virtual int get_output_index(const vector<double> & input )
	{
		...
		for(itor = weakers.begin();itor != weakers.end();itor++)
		{
			try
			{
				...
				d_sign += ((idx==1)?-1:1) * (itor->second + 0.01);
			}
			catch (cls_exception &exc)
			{
				try
